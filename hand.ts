...
export class Hand implements HandInterface {
    ...
    private givePots = (winners: {
        ...
        let pot = this.pots?.[0]?.amount!
        const summWinning = Math.ceil(pot / winners.playerIds.length);
    ...
    act(playerId: PlayerId, action: PlayerAction) {
        if (playerId !== this.currentPlayer) {
            throw new Error("Not your turn" + "   " + playerId + "   " + this.currentPlayer);
        }
    ...
    isValidBet(playerId: PlayerId, amount: number) {
        ...
        if (this.getOtherMinBet(playerId) === nextBet) {
            return true
        }
    ...
    private getOtherMinBet(currentUser: PlayerId) {
        if (Object.keys(this.bets).length === 0) {
            return 0
        }
        const others = {...this.bets}
        delete others[currentUser]
        return Math.min(...Object.values(others))
    }
